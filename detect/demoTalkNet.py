...
parser.add_argument('--cropScale',             type=float, default=0.80, help='Scale bounding box')

parser.add_argument('--start',                 type=int, default=18*60,   help='The start time of the video')
parser.add_argument('--duration',              type=int, default=19*60,  help='The duration of the video, when set as 0, will extract the whole video')
...
def scene_detect(args):
    video = open_video(args.videoPath)
...
def inference_video(args):
	...
	app = FaceAnalysis(name="buffalo_sc",providers=['CUDAExecutionProvider'], allowed_modules=['detection'])
	...
	for fidx, fname in enumerate(flist):
		...
		if fidx ==0:
			h,w = imageNumpy.shape[:2]
			h = (h // 32) * 32
			w = (w //32) * 32
			app.prepare(ctx_id=0, det_size=(h,w), det_thresh=0.4)
		faces = app.get(imageNumpy)
		dets.append([])
		for face in faces:
			bbox = face['bbox']
			conf = face['det_score']
			dets[-1].append({'frame':fidx, 'bbox':(bbox).tolist(), 'conf':conf}) # dets has the frames info, bbox info, conf info
...
def main(path_to_file = None):
	...
	if path_to_file != None:
		args.videoPath = path_to_file
		video_folder = os.path.dirname(args.videoPath)
		video_name = os.path.splitext(os.path.basename(args.videoPath))[0]
		args.savePath = os.path.join(video_folder, video_name)
